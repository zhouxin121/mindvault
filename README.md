...
> 开源 Agent 记忆系统：对话长期归档 · 规则自动萃取 · 上下文快速恢复 · 可选五步思考协议。
> 5000+ 轮实战验证，跨平台支持 OpenClaw / Marvis / CherryStudio / Coze 等主流 Agent 框架。

MindVault 是一套**完整的 Agent 记忆与思考基础设施**，解决 AI Agent 长期使用中的核心痛点：对话窗口溢出后内容丢失、每次新会话需要重新教规则、中断后无法快速恢复项目上下文。通过三层架构（保险库 → 进化引擎 → 项目快照），MindVault 让 Agent 拥有"长期记忆"和"结构化思考"能力，且所有数据以 JSONL 开放格式存储，数据主权掌握在用户手中。
